...
def rewrite_query(user_query: str, past_conversations: List[Dict]) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """Rewrite the query to improve document retrieval while preserving the original intent.
...
Rewritten query:""",
    ...
    chain = prompt | llm
    result = chain.invoke(
        {"query": user_query, "past_conversations": past_conversations}
    )
...
def refine_query_for_retry(original_query: str, retrieved_chunks: List[str]) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a query refinement assistant. When a search query doesn't retrieve relevant documents 
    ...
    result = chain.invoke(
        {
            ...
            "sample_content": (
                sample_content[:500] if sample_content else "No content retrieved"
            ),
        }
    )
