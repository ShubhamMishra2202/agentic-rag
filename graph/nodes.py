...
from utils.stop_conditions import (
    is_goodbye_message,
...
def intent_classify_node(state: GraphState) -> GraphState:
    ...
    if not query:
        logger.warning("Empty query, defaulting to retrieval_required")
...
def direct_answer_node(state: GraphState) -> GraphState:
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a helpful assistant. Answer the user's question directly without needing to retrieve documents.
...
def retrieve_node(state: GraphState) -> GraphState:
    ...
    if len(messages) > 0:
        ...
        for msg in messages[-10:]:  # Last 10 messages
            if hasattr(msg, "type") and hasattr(msg, "content"):
                past_conversations.append({"role": msg.type, "content": msg.content})
    ...
    if chunks_with_scores:
        ...
        if top_score >= relevance_threshold:
            is_relevant = True
            logger.info(
                f"Relevance check: score {top_score:.4f} >= threshold {relevance_threshold:.4f}"
            )
        else:
            logger.warning(
                f"Relevance check: score {top_score:.4f} < threshold {relevance_threshold:.4f}"
        ...
        logger.warning("No scores found in chunks, assuming relevant")
        is_relevant = True
    ...
    if not is_relevant and context_list:
        ...
        try:
            ...
            logger.debug(f"Refined query: '{original_query}' -> '{refined_query}'")
            ...
            if retry_scores:
                ...
                if retry_top_score >= relevance_threshold:
                    ...
                    logger.info(
                        f"Retry successful: score {retry_top_score:.4f} >= threshold {relevance_threshold:.4f}"
                    )
                else:
                    logger.warning(
                        f"Retry still not relevant: score {retry_top_score:.4f} < threshold {relevance_threshold:.4f}"
                    )
    ...
    for msg in messages:
        ...
        elif isinstance(msg, AIMessage) and not (
...
def answer_node(state: GraphState) -> GraphState:
    ...
    if state.get("should_stop", False):
        logger.info(
            f"Stopping conversation - reason: {state.get('stop_reason', 'unknown')}"
        )
    ...
    if needs_fallback or not context:
        logger.warning("Using fallback message - no relevant chunks found")
    ...
    try:
        ...
        if is_complete:
            logger.debug("Answer determined to be complete - appending closing message")
            ...
            logger.debug("Answer may be incomplete - no closing message added")
    except Exception as e:
        logger.warning(
            f"Error checking answer completeness: {e} - continuing without check"
        )
    ...
    state["answer"] = answer
    state["messages"] = (
        messages  # Persist chat history (only user messages and final answers)
    )
    return state
